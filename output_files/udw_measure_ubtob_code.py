...
os.environ['UDW_MEASURE_UBTOB_CODE_PATH'] = 's3a://delta-lake-tables/udw/measure_ubtob_code.csv'
os.environ['MEASURE_UBTOB_CODE_DELTA_TABLE_PATH'] = 's3a://delta-lake-tables/delta-tables/udw/measure_ubtob_code'
...
def main():
    ...
    try:
        ...
        measure_ubtob_code_src_file_path = os.environ.get("UDW_MEASURE_UBTOB_CODE_PATH", "")
        ...
        measure_ubtob_code_df = src_df \
            ...
            .withColumn("renedering_provider_id", get_column(src_df, "RenederingProviderID", LongType())) \
            .select([
                "id",
                "health_record_key",
                "code",
        ...
        spark.sql("""
            CREATE TABLE IF NOT EXISTS udw.measure_ubtob_code (
                ...
                renedering_provider_id LONG
            )
            USING delta
            PARTITIONED BY (client_id)
            LOCATION 's3a://delta-lake-tables/delta-tables/udw/measure_ubtob_code'
        """)
