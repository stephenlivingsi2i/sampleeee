...
os.environ['UDW_MEASURE_HIGH_BP_CODE_PATH'] = 's3a://delta-lake-tables/udw/measure_high_bp_code.csv'
os.environ['MEASURE_HIGH_BP_CODE_DELTA_TABLE_PATH'] = 's3a://delta-lake-tables/delta-tables/udw/measure_high_bp_code'
...
def main():
    app_name = "udw_measure_high_bp_code_load"
    ...
    try:
        ...
        measure_high_bp_code_src_file_path = os.environ.get("UDW_MEASURE_HIGH_BP_CODE_PATH", "")
        if not measure_high_bp_code_src_file_path:
            exit_with_error(logger, f"measure_high_bp_code source file path should be provided!")

        measure_high_bp_code_delta_table_path = os.environ.get("MEASURE_HIGH_BP_CODE_DELTA_TABLE_PATH", "")
        if not measure_high_bp_code_delta_table_path:
            exit_with_error(logger, f"measure_high_bp_code delta table path should be provided!")

        # read measure_high_bp_code source csv file from s3 bucket
        logger.warn(f"read measure_high_bp_code source csv file from s3")
        src_df = spark.read.csv(measure_high_bp_code_src_file_path, header=True)
        measure_high_bp_code_df = src_df \
        ...
        spark.sql("""
            CREATE TABLE IF NOT EXISTS udw.measure_high_bp_code (
                id INTEGER,
            ...
            LOCATION 's3a://delta-lake-tables/delta-tables/udw/measure_high_bp_code'
        """)

        # write data into delta lake measure_high_bp_code table
        measure_high_bp_code_df.write.format("delta").mode("append").save(measure_high_bp_code_delta_table_path)
