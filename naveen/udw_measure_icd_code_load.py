...
# export UDW_MEASURE_ICD_CODE_FILE_PATH="s3a://sureshn-sandbox-landing-bucket/udw/MEASURE_ICD_CODE.csv"
# export MEASURE_ICD_CODE_DELTA_TABLE_PATH="s3a://sureshn-sandbox-landing-bucket/delta-tables/udw/MEASURE_ICD_CODE"


def main():
    app_name = "udw_measure_icd_code_load"
    ...
    try:
        ...
        measure_icd_code_src_file_path = os.environ.get("UDW_MEASURE_ICD_CODE_FILE_PATH", "")
        if not measure_icd_code_src_file_path:
            exit_with_error(logger, f"measure_icd_code source file path should be provided!")

        measure_icd_code_delta_table_path = os.environ.get("MEASURE_ICD_CODE_DELTA_TABLE_PATH", "")
        if not measure_icd_code_delta_table_path:
            exit_with_error(logger, f"measure_icd_code delta table path should be provided!")

        # read measure_icd_code source csv file from s3 bucket
        logger.warn(f"read measure_icd_code source csv file from s3")
        src_df = spark.read.csv(measure_icd_code_src_file_path, header=True)
        
        measure_icd_code_df = src_df \
        ...
        spark.sql("""
            CREATE TABLE IF NOT EXISTS udw.measure_icd_code(
                id INTEGER,
        ...
        # write data into delta lake measure_icd_code table
        measure_icd_code_df.write.format("delta").mode("append").save(measure_icd_code_delta_table_path)
